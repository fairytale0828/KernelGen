"""
Agent基类 - 基于langchain的智能Agent基础实现，参考aikg架构
"""
...
from ..core.llm.model_loader import create_model
from ..utils.common_utils import get_prompt_path
...
kernelgen_stream_output = os.getenv("KERNELGEN_STREAM_OUTPUT", "off").lower() == "on"


class AgentBase(ABC):
    """KernelGen代理基类，提供基础功能和接口"""
    ...
    async def run_llm(self, prompt: PromptTemplate, input: Dict[str, Any], model_name: str) -> tuple[str, str, str]:
        ...
        try:
            ...
            if not kernelgen_stream_output:
